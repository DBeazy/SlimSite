...
abstract class Content implements ActiveRecordInterface 
{
    ...
    public function setPageName($v)
    {
        ...
        if ($this->page_name !== $v) {
            $this->page_name = $v;
            $this->modifiedColumns[ContentTableMap::COL_PAGE_NAME] = true;
        }
    ...
    public function setContent($v)
    {
        ...
        if ($this->content !== $v) {
            $this->content = $v;
            $this->modifiedColumns[ContentTableMap::COL_CONTENT] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ContentTableMap::translateFieldName('PageName', TableMap::TYPE_PHPNAME, $indexType)];
            $this->page_name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ContentTableMap::translateFieldName('Content', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = ContentTableMap::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ContentTableMap::COL_PAGE_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'page_name';
        }
        if ($this->isColumnModified(ContentTableMap::COL_CONTENT)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'id':                        
                        ...
                        break;
                    case 'page_name':                        
                        $stmt->bindValue($identifier, $this->page_name, PDO::PARAM_STR);
                        break;
                    case 'content':                        
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ContentTableMap::COL_PAGE_NAME)) {
            $criteria->add(ContentTableMap::COL_PAGE_NAME, $this->page_name);
        }
        if ($this->isColumnModified(ContentTableMap::COL_CONTENT)) {
            $criteria->add(ContentTableMap::COL_CONTENT, $this->content);
        }
